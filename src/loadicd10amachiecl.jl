function importicd10amcodes(fname)
  ...
  sex = recode(categorical(icdcodes.sex; levels=1:2, compress=true), 1 => "male",
               2 => "female")
  Stype = recode(categorical(icdcodes.Stype; levels=1:2, compress=true), 1 => "fatal",
                 2 => "warning")
  AgeL = passmissing(ICD10AMAge).(icdcodes.AgeL)
  AgeH = passmissing(ICD10AMAge).(icdcodes.AgeH)
  Atype = recode(categorical(icdcodes.Atype; levels=1:2, compress=true), 1 => "fatal",
                 2 => "warning")
  ...
  global _ICD10AMcodes_ = Table(; level=Level, icdcode=code_id, dagger=dagger,
                                asterisk=asterisk, validforcoding=valid,
                                australiancode=aust_code, description=ascii_desc,
                                shortdescription=ascii_desc_short,
                                effectivefrom=effective_from, inactive=inactive,
                                reactivated=reactivated, sex=sex, stype=Stype, agel=AgeL,
                                ageh=AgeH, atype=Atype, rarenotifiabledx=RDiag,
                                morphcoderequired=Morph_Code,
                                conceptchange=concept_change, unacceptpdx=UnacceptPDx)
  return save(normpath(@__DIR__, "..", "data", "icd10amcodes.jld2"),
...
function importachicodes(fname)
  ...
  sex = recode(categorical(achicodes.sex; levels=1:2, compress=true), 1 => "male",
               2 => "female")
  Stype = recode(categorical(achicodes.Stype; levels=1:2, compress=true), 1 => "fatal",
                 2 => "warning")
  AgeL = passmissing(ICD10AMAge).(achicodes.AgeL)
  AgeL = passmissing(ICD10AMAge).(achicodes.AgeL)
  AgeH = passmissing(ICD10AMAge).(achicodes.AgeH)
  Atype = recode(categorical(achicodes.Atype; levels=1:2, compress=true), 1 => "fatal",
                 2 => "warning")
  global _ACHIcodes_ = Table(; achicode=Code_id, block=Block, description=ascii_desc,
                             shortdescription=ascii_short_desc,
                             effectivefrom=effective_from, inactive=inactive, sex=sex,
                             stype=Stype, agel=AgeL, ageh=AgeH, atype=Atype)
  return save(normpath(@__DIR__, "..", "data", "achicodes.jld2"),
